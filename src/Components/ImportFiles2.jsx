...
export default function LegalTextManager() {
  ...
  // Nouvelle logique pour analyser le fichier CSV des législations
  const analyzeLegislationCSV = (results) => {
    const headers = results.meta.fields
    const requiredHeaders = ["Titre_legislation", "Date d'entrée en vigueur", "Date dernière modification"]
    const structureHeaders = ["Titre", "Chapitre", "Section"]
    ...
    const legislations = results.data.reduce((acc, row) => {
      const key = `${row.Titre_legislation}-${row["Date d'entrée en vigueur"]}-${row["Date dernière modification"]}`
      if (!acc[key]) {
        acc[key] = {
      ...
      let currentLevel = acc[key].structure
      for (const header of structureHeaders) {
        if (row[header]) {
          const existingItem = currentLevel.find(item => item.type === header.toLowerCase() && item.content === row[header])
          if (!existingItem) {
            const newItem = { type: header.toLowerCase(), content: row[header], children: [] }
    ...
    return Object.values(legislations)
  }
      ...
      Papa.parse(uploadedFile, {
        ...
        complete: (results) => {
          if (selectedType === "Législation") {
            const legislations = analyzeLegislationCSV(results)
            if (legislations) {
              setParsedTexts(legislations)
              setStructure(legislations[0]?.structure || [])
              setError(null)
            }
          } else {
            // Logique existante pour les autres types de textes
            ...
            } else if (!cleanedData[0].hasOwnProperty('Title') || !cleanedData[0].hasOwnProperty('Content')) {
                  ...
                  {selectedType === "Législation" ? (
                    <div className="mt-2">
                      ...
                      <ul className="list-disc list-inside pl-4">
                        {text.structure.map((item, idx) => (
                          <li key={idx}>{item.type}: {item.content}</li>
                        ))}
                      </ul>
                    </div>
                  ) : (
