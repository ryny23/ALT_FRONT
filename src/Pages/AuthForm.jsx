...
const AuthForm = () => {
  ...
  const onFormSubmit = (event) => {
    ...
    const registerData = {
      ...
      email: email,
      password: password,
      profession: selectedProfession, // Ajouter la profession sélectionnée
  };
  ...
  if (conLoggedIn || localStorage.getItem('token') || Cookies.get('token')) {
    ...
    return (
        <div className="w-full h-screen flex items-start">
            ...
            <div className={`w-1/2 h-full bg-[white] flex flex-col p-10 justify-items-start transition-transform duration-700 transform ${activeTab === 'register' ? 'translate-x-full' : 'translate-x-0'}`}>
                ...
                {activeTab === 'login' && conError && <div className="text-red-500">{conError}</div>}
                {activeTab !== 'login' && error && <div className="text-red-500">{error}</div>}
